...
pub async fn load<'a>(client: &Client, cache: cache::Entry<'a>, project_id: &str, game_version: &Option<String>, transform: &config::Transform) -> Result<cache::Reference> {
...
async fn resolve_version(client: &Client, project_id: &str, game_version: &Option<String>) -> Result<Option<(String, String, String)>> {
    ...
    for version in versions {
        ...
        if let Some(file) = file {
            if let Some(hash) = &file.hashes.sha512 {
                return Ok(Some((hash.clone(), file.url.clone(), file.filename.clone())));
            } else {
...
impl Client {
    ...
    async fn get_versions(&self, project_id: &str, game_version: &Option<String>) -> Result<Vec<ProjectVersion>> {
        let url = if let Some(game_version) = game_version {
            format!("{}/v2/project/{}/version?game_version={}", Client::BASE_URL, project_id, game_version)
        } else {
